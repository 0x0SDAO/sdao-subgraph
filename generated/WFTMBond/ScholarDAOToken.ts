...
export class ScholarDAOToken extends ethereum.SmartContract {
  static bind(address: Address): ScholarDAOToken {
    return new ScholarDAOToken("ScholarDAOToken", address);
  }
  ...
  psdao(): Address {
    let result = super.call("psdao", "psdao():(address)", []);

    return result[0].toAddress();
  }

  try_psdao(): ethereum.CallResult<Address> {
    let result = super.tryCall("psdao", "psdao():(address)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
...
export class ClaimWithPSDAOCall extends ethereum.Call {
  get inputs(): ClaimWithPSDAOCall__Inputs {
    return new ClaimWithPSDAOCall__Inputs(this);
  }

  get outputs(): ClaimWithPSDAOCall__Outputs {
    return new ClaimWithPSDAOCall__Outputs(this);
  }
}

export class ClaimWithPSDAOCall__Inputs {
  _call: ClaimWithPSDAOCall;

  constructor(call: ClaimWithPSDAOCall) {
    this._call = call;
  }
}

export class ClaimWithPSDAOCall__Outputs {
  _call: ClaimWithPSDAOCall;
